...
add_action('init', 'puertos_register');
function puertos_register() {
    $args = array(
        'label' => 'Puertos',
        'singular_label' => 'Puerto',
        'public' => true,
		'menu_position' => 5, 
        '_builtin' => false,
        'capability_type' => 'post',
		'has_archive' => false,
        'hierarchical' => false,
        'rewrite' => array( 'slug' => 'que-hacemos/terminales'),
        'supports' => array('title', 'excerpt' , 'thumbnail' )
    );
    register_post_type('puertos', $args);
    flush_rewrite_rules();
}

register_taxonomy('terminal', array('puertos'), array("hierarchical" => true, "label" => "Tipos", "singular_label" => "Tipo", "rewrite" => 'hierarchical'));
...
register_taxonomy('area', array('staff'), array("hierarchical" => true, "label" => "Áreas", "singular_label" => "Área", "rewrite" => 'hierarchical'));
...
function videos_register() {
    $args = array(
		...
		'has_archive' => true,
        'hierarchical' => false,
        'rewrite' => array( 'slug' => 'nuestra-filosofia/videos'),
        'supports' => array('title', 'editor' , 'excerpt' , 'thumbnail' )
    );
...
add_action('init', 'reportes_anuales');
function reportes_anuales() {
    $args = array(
        'label' => 'Reportes Anuales',
        'singular_label' => 'Reporte',
        'public' => true,
		'menu_position' => 10, 
        '_builtin' => false,
        'capability_type' => 'post',
		'has_archive' => false,
        'hierarchical' => false,
        'rewrite' => array( 'slug' => 'inversionistas/reportes-anuales'),
        'supports' => array('title', 'excerpt' , 'thumbnail' )
    );
    register_post_type('reportes_anuales', $args);
    flush_rewrite_rules();
}
